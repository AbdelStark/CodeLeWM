...
class DemoTerminalReportTest(unittest.TestCase):
    def test_renderer_summarizes_pipeline_without_candidate_patch_text(self) -> None:
        ...
        self.assertIn("World-model score trace", output)
        self.assertIn("all candidates worse than no-op: false", output)
        self.assertIn("latent details: not_recorded", output)
