...
class RetrievalCliTest(unittest.TestCase):
    def test_retrieval_cli_writes_report_and_manifested_artifact(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            report = json.loads((out_dir / "reports" / "retrieval_report.json").read_text(encoding="utf-8"))
        ...
        self.assertEqual(payload["schema_version"], RETRIEVAL_EVAL_RUN_SCHEMA_VERSION)
        ...
        self.assertEqual(
            ...
            ["evaluation.retrieval.start", "evaluation.retrieval.complete"],
        )

    def test_retrieval_cli_rejects_existing_output_without_overwrite(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            out_dir = root / "runs" / "retrieval"
...
def _train_tiny_fixture(root: Path, pack_dir: Path) -> Path:
    ...
    payload["name"] = "retrieval_cli_fixture"
