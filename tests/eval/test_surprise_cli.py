...
class SurpriseCliTest(unittest.TestCase):
    def test_surprise_cli_writes_report_and_manifested_artifact(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            out_dir = root / "runs" / "surprise"
            log_path = root / "surprise.jsonl"
            ...
            report = json.loads((out_dir / "reports" / "surprise_report.json").read_text(encoding="utf-8"))
        ...
        self.assertEqual(payload["schema_version"], SURPRISE_EVAL_RUN_SCHEMA_VERSION)
        ...
        self.assertIn("mutation", report["metrics"]["pairwise_auc_by_category"])
        self.assertGreaterEqual(report["examples"][0]["true_rank"], 1)
        ...
        self.assertEqual({path.name for path in checked_files}, {"config.json", "surprise_report.json"})
        self.assertEqual(
            [event["event"] for event in log_events],
            ["evaluation.surprise.start", "evaluation.surprise.complete"],
        )

    def test_surprise_cli_rejects_existing_output_without_overwrite(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            out_dir = root / "runs" / "surprise"
...
def _train_tiny_fixture(root: Path, pack_dir: Path) -> Path:
    ...
    payload["name"] = "surprise_cli_fixture"
