...
class PullRequestWorkflowContractTest(unittest.TestCase):
    ...
    def test_workflow_builds_and_installs_package_artifacts(self) -> None:
        ...
        self.assertIn("uv run pip-audit --format json --output .ci/dependency-audit/pip-audit.json", text)
        self.assertIn("scripts/release-provenance", text)
        self.assertIn("DEPENDENCY_PROVENANCE.md", text)
