...
class FinalPaperPackageTest(unittest.TestCase):
    ...
    def test_paper_matches_final_claim_boundary(self) -> None:
        ...
        for marker in (
            ...
            "RQ1: Does action conditioning improve code-transition",
            "RQ2: Can an execution-trace substrate support non-collapsed",
            "RQ3: Does the learned world model improve downstream candidate",
            "Text-action Recall@1 was 0.263",
            "no-action reached Recall@1 0.441",
            "deterministic mutation-distractor reranking pack",
            "WS-D denotes the mutation-distractor reranking benchmark",
            "128 problem-slice rows and 768 candidate completions",
            "clean-checkout replay over persisted v0.9 score rows, not a new scoring run",
            "HumanEval WS-D",
            "0.9787",
            "+10.64",
            "+8.51",
            "Figure~\\ref{fig:downstream}",
            "\\label{fig:downstream}",
            "MBPP-Plus is the counterweight",
            "1.0000 & 1.0000 & 0.1765 & 1.0000 & +0.00",
            "No-action is a scientific baseline, not a convenience baseline.",
            "Outlook: What Would Make the Next Claim Stronger?",
            "non-saturated downstream labels",
    ...
    def test_claim_audit_blocks_unsupported_claims(self) -> None:
        ...
        for marker in (
            ...
            "128 problem-slice rows and 768",
