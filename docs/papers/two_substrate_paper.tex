...
\begin{tabular}{lrrrl}
\toprule
Run & mutation & same-code/input & same-problem/submission & gate \\
\midrule
v0.6 seed 42 & 1.000 / 236 & 1.000 / 352 & 1.000 / 6 & closed \\
v0.6 seed 1729 & 1.000 / 236 & 1.000 / 352 & 1.000 / 6 & closed \\
\bottomrule
\end{tabular}
\end{table}
...
the only evaluable semantic target. The semantic-surprise score result is also
limited by count gates: same-code/different-input decoys are plentiful, but
same-problem/different-submission remains small-n. The right conclusion is not
that \modelname{} solves code generation; it is that a JEPA-style latent
transition model can learn an action-conditioned execution substrate when the
substrate supplies precise state/action/next-state signal.
