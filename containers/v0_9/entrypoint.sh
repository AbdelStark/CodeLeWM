#!/usr/bin/env bash
# CodeLeWM v0.9 runtime container entrypoint.
#
# The entrypoint pre-downloads the execution pack, forwards the training command,
# and uploads the run output directory after a successful command. Every runtime
# phase emits CODELEWM_JOB_EVENT JSON so operators can summarize live jobs and
# persisted reports/job_progress.jsonl with scripts/hf-job-event-status.
...
hf upload \
    ...
    --repo-type dataset \
    --revision main \
    --commit-message "codelewm v0.9 run ${CODELEWM_HF_RUN_NAME:-unset}"
UPLOAD_EXIT_CODE=$?
set -e
