...
- `codelewm.eval`: action-view report policy, collapse diagnostics, evaluation
  gates, and kill-report artifacts;
...
The manifest-backed training runner, retrieval evaluator, patch scorer, reranker,
and release CI are still being implemented. Root `train.py`, `eval.py`, and the
existing Hydra configs are inherited from the original LeWorldModel seed and are
kept for compatibility while the CodeLeWM-specific runtime lands under the
package.
